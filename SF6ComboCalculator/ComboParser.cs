...
public class ComboParser
{
    ...
    public ComboParserResult Parse(string comboNotation, CharacterStates? states = null)
    {
        ...
        for (var index = 0; index < splittedString.Length; index++)
        {
            ...
            var attack = adapter.Adapt(FindAttack(cleanString, ref numberOfStocks));
    ...
    private AttackModel FindAttack(string notation, ref int numberOfStocks)
    {
        if (numberOfStocks != 0 && !notation.Contains("{s}") && StockEnhancedAttackOf(notation) is not null)
        {
            var attack = StockEnhancedAttackOf(notation);
            numberOfStocks--;
    ...
    private AttackModel? StockEnhancedAttackOf(string notation)
    {
        var tempNotation = notation.Trim() + "{s}";
        var attack =
            _attacks.FirstOrDefault(x => x.Notation.Equals(tempNotation, StringComparison.InvariantCultureIgnoreCase)) ??
            _attacks.FirstOrDefault(x =>
                x.Aliases.Contains(tempNotation, StringComparer.InvariantCultureIgnoreCase));
        return attack;
    }
    ...
    private static (string CleanString, 
                    ...
                    bool IsCounterHit) CleanupAttackString(string str, int index)
    {
